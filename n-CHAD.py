...
"""
 SYNOPSIS:
   ./confirmablelorawan.py <nodes> <avgsend> <datasize> <collision> <randomseed> <p> <nrHops> <packetsize>
 DESCRIPTION:
    nodes
        ...
        for other in packetsAtBS:
            if node.packet.nodeid != other.packet.nodeid: # nodes are different
                if frequencyCollision(node.packet, other.packet) and sfCollision(node.packet, other.packet):
                    if node.cadStart >= other.packet.addTime and node.cadStart + node.packet.cadtime <= other.packet.addTime + other.packet.pretime: # 5 sym cad time in preamble time
                        if checkRange(node, other): # nodes in hearing range last for efficiency
                            return False # shortcircuit at first preamble detected. Efficiency in large network
    return True
...
class myPacket():
    def __init__(self, nodeid, freq, sf, bw, cr, txpow, distance):
        ...
        self.pretime = preambleTime(self.sf,self.bw)
...
def transmit(env,node):
    while node.buffer > 0.0:
        ...
        elif (node.bckretans and node.bckretans <= maxBackoffs):
            node.first = 0
            node.buffer += PcktLength_SF[node.parameters.sf-7]
            # the randomization part (2 secs) to resove the collisions among retrasmissions
            yield env.timeout(random.uniform(0, node.packet.rectime))
        else:
            node.first = 0
            node.lstretans = 0
            node.bckretans = 0
        ...
        if tryToSend: # TODO two-step check unecessary for non persistent
            ...
            # TODO
            # we need an alternative here in the case that a node chooses not to send
            # I suggest simply adding back into the buffer (or same as below)
        else:
...
if len(sys.argv) >= 10:
    ...
    persist = float(sys.argv[6])
    nrHops = int(sys.argv[7])
    packetsize = int(sys.argv[8])
    maxBackoffs = int(sys.argv[9])
    print "Nodes:", nrNodes
    ...
    print "usage: ./confirmablelorawan <nodes> <avgsend> <datasize> <collision> <randomseed> <p> <nrHops> <packetsize> <maxBackoffs>"
    exit(-1)
...
# redefine packetsize
PcktLength_SF = [packetsize for i in range(6)]
...
try:
    ...
    failFreq = 880
    winsound.Beep(failFreq, duration)
...
if os.path.isfile(fname):
     ...
     res = "randomseed, collison, nodes, avgSendTime, datasize, nrTransmissions, nrCollisions, nrBackoffs, nrlost, nrlosterror, nrNoACK, nrACKLost, CollectionTime, DER1, DER2, OverallEnergy, nodefair1, nodefair2, sfdistribution\n" + str(sys.argv[5]) + ", " + str(full_collision) + ", " + str(nrNodes) + ", " + str(avgSendTime) + ", " + str(datasize) + ", " + str(sent) + ", "  + str(nrCollisions) + ", " + str(nrBackoffs) + ", " + str(nrLost) + ", "  + str(nrLostError) + ", " +str(nrNoACK) + ", " +str(nrACKLost) + ", " + str(env.now)+ ", " + str(der1) + ", " + str(der2)  + ", " + str(energy) + ", "  + str(nodefair1) + ", "  + str(nodefair2) + ", "  + str(SFdistribution)
