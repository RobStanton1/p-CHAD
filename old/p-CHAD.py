...
def performCAD(node):
    ...
    if packetsAtBS:
        for other in packetsAtBS:
            if node.packet.nodeid != other.packet.nodeid: # nodes are t
                print "comparing nodes: ", node.nodeid, other.nodeid
                if frequencyCollision(node.packet, other.packet) and sfCollision(node.packet, other.packet):
                    timeCheck = node.cadStart if node.sensing == 1 else node.cadStart + node.packet.cadtime
                    if timeCheck < other.packet.addTime + other.packet.pretime:                                             
                        if checkRange(node, other): # nodes in hearing range last for efficiency
                            print node.cadStart + node.packet.cadtime, other.packet.addTime + other.packet.pretime, (other.packet.addTime + other.packet.pretime) - (node.cadStart + node.packet.cadtime)
                            wait.append((other.packet.addTime + other.packet.pretime) - timeCheck)
                            #raw_input("continue..")
        # end for
        return max(wait) if len(wait) > 0 else 0 # of preambles detected return the maximum time to wait until preamble is over
    # end if
    return 0
...
def transmit(env,node):
    while node.buffer > 0.0:
        ...
        yield env.timeout(node.packet.cadtime) # waits 5 symbols on first cad
        node.rxtime += node.packet.cadtime
        TtpreFinish = performCAD(node) # returns time to preamble finish or 0
        ...
        if TtpreFinish == 0:
            ...
            while TtpreFinish != 0:
                ...
                yield env.timeout(TtpreFinish) # wait until preamble finishes
                node.rxtime += TtpreFinish  # add rx time for energy calc
                node.cadStart = env.now # update current time
                node.sensing = 1                 
                TtpreFinish = performCAD(node) # another packet may have arrived in mean time to cad again. no 5 sym lock on this time
