...
# check if the gateway can ack this packet at 10% channel only
# based on the duy cycle
...
def sfCollision(p1, p2):
    if p1.sf == p2.sf:
        # "collision sf node {} and node {}".format(p1.nodeid, p2.nodeid)
...
#
# this function computes the the preamble time of a packet
# Rob addition
#
...
def performCAD(node):
    ...
    if packetsAtBS:
        for other in packetsAtBS:
            ...
            if node.packet.nodeid != other.packet.nodeid: # nodes are different

                print "comparing nodes: ", node.packet.nodeid, " ", other.packet.nodeid 

                if frequencyCollision(node.packet, other.packet) and sfCollision(node.packet, other.packet):
                    
                    print "frequencyCollision: ", frequencyCollision(node.packet, other.packet), "sfCollision: ", sfCollision(node.packet, other.packet)
                    # print frequencies
                    print "node ", node.packet.nodeid, "freq: ", node.packet.freq, "sf: ", node.packet.sf
                    print "node ", other.packet.nodeid, "freq: ", other.packet.freq, "sf: ", other.packet.sf

                    # print node cad start time and other arrival time 
                    print "node ", node.packet.nodeid, "cadStart: ", node.cadStart
                    print "node ", other.packet.nodeid, "addTime: ", other.packet.addTime

                    # print node cad finish time and other node preamble finish
                    print "node ", node.packet.nodeid, "cadFinish: ", node.cadStart + node.packet.cadtime
                    print "node ", other.packet.nodeid, "preFinish: ", other.packet.addTime + other.packet.pretime

                    if node.cadStart >= other.packet.addTime and node.cadStart + node.packet.cadtime <= other.packet.addTime + other.packet.pretime:
                    ...
                    print "no freq/sf collision"
        # end for
        print "packetsAtBS checked"
        print "channel free: ", free
        #raw_input("continue...")
        #return free
    # end if
    print "len(packetsAtBS): ", len(packetsAtBS)
    #raw_input("continue...")
    return True
...
def transmit(env,node):
    while node.buffer > 0.0:
        ...
        elif (node.bckretans and node.bckretans <= 8):
            node.first = 0
            #node.buffer += PcktLength_SF[node.parameters.sf-7]
            # the randomization part (2 secs) to resove the collisions among retrasmissions
            yield env.timeout(max(2+airtime(12, CodingRate, AckMessLen+LorawanHeader, Bandwidth), float(node.packet.rectime*((1-0.01)/0.01)))+(random.expovariate(1.0/float(2000))/1000.0))
        ...
        print "node ", node.nodeid, " waking up: "
        tryToSend = False
        hopCount = 0
        node.cadStart = env.now
        print "node ", node.nodeid, " performing CAD: ", node.cadStart        
        ...
        if channelFree:
            ...
            while hopCount < nrHops:
                ...
                # cad again
                node.cadStart = env.now 
                print "node ", node.nodeid, " performing CAD: ", node.cadStart#, "this should match cad finish above"
                #raw_input("continue...")  
                yield env.timeout(node.packet.cadtime)
                node.rxtime += node.packet.cadtime
                channelFree = performCAD(node)
        ...
        if tryToSend: # TODO two-step check unecessary for non persistent
...
if os.path.isfile(fname):
     res= "\n" + str(sys.argv[5]) + ", " + str(full_collision) + ", " + str(nrNodes) + ", " + str(avgSendTime) + ", " + str(datasize) + ", " + str(sent) + ", "  + str(nrCollisions) + ", " + str(nrBackoffs) + ", " + str(nrLost) + ", "  + str(nrLostError) + ", " +str(nrNoACK) + ", " +str(nrACKLost) + ", " + str(env.now)+ ", " + str(der1) + ", " + str(der2)  + ", " + str(energy) + ", "  + str(nodefair1) + ", "  + str(nodefair2) + ", "  + str(SFdistribution)
else:
     res = "#randomseed, collType, nrNodes, TransRate, DataSize, nrTransmissions, nrCollisions, nrBackoffs, nrlost, nrlosterror, nrnoack, nracklost, CollectionTime, DER1, DER2, OverallEnergy, nodefair1, nodefair2, sfdistribution\n" + str(sys.argv[5]) + ", " + str(full_collision) + ", " + str(nrNodes) + ", " + str(avgSendTime) + ", " + str(datasize) + ", " + str(sent) + ", "  + str(nrCollisions) + ", " + str(nrBackoffs) + ", " + str(nrLost) + ", "  + str(nrLostError) + ", " +str(nrNoACK) + ", " +str(nrACKLost) + ", " + str(env.now)+ ", " + str(der1) + ", " + str(der2)  + ", " + str(energy) + ", "  + str(nodefair1) + ", "  + str(nodefair2) + ", "  + str(SFdistribution)
